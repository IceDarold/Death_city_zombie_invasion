...
namespace Zombie3D
{
	public class TankGraphPathFinding : IPathFinding
	{
		public Stack<Transform> FindPath(Vector3 enemyPos, Vector3 playerPos)
		{
			...
			foreach (GameObject gameObject in array)
			{
				...
				if (magnitude < num)
				{
					...
					if (!Physics.Raycast(ray, out raycastHit, magnitude, 34816))
					{
		...
		public Stack<Transform> SearchPath(WayPointScript from, WayPointScript to)
		{
			...
			{
				UnityEngine.Debug.Log("To small house");
				WayPointScript wayPointByName = this.GetWayPointByName("WayPoint6");
				to = wayPointByName;
			}
			if (from == to)
			{
