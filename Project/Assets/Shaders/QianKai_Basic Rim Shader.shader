Shader "QianKai/Basic Rim Shader"
{
  Properties
  {
  ...
  SubShader
  {
    ...
    Pass // ind: 1, name: FORWARDBASE
    {
      ...
      OUT_Data_Frag frag(v2f in_f)
      {
          ...
          float4 tmpvar_6;
          tmpvar_6.w = 1;
          tmpvar_6.xyz = ((((max(0, dot(normalize(in_f.xlv_NORMAL), normalize(_WorldSpaceLightPos0.xyz))) * _LightColor0.xyz) + (glstate_lightmodel_ambient * 2).xyz) * (tmpvar_4.xyz * _MainColor.xyz)) + ((_RimColor.xyz * pow(Rim_2, _RimPower)) * _RimIntensity));
          tmpvar_1 = tmpvar_6;
