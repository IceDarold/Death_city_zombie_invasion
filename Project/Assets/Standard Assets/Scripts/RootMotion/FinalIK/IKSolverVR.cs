	...
	public class IKSolverVR : IKSolver
	{
		...
		public class Spine : IKSolverVR.BodyPart
		{
			...
			protected override void OnRead(Vector3[] positions, Quaternion[] rotations, bool hasChest, bool hasNeck, bool hasShoulders, bool hasToes, int rootIndex, int index)
			{
				...
				Vector3 position = positions[index + 2];
				Quaternion quaternion3 = rotations[index + 2];
				Vector3 position2 = positions[index + 3];
				Quaternion rotation = rotations[index + 3];
				Vector3 vector3 = positions[index + 4];
				...
				if (!this.initiated)
				{
					...
					this.headHeight = V3Tools.ExtractVertical(vector3 - positions[0], rotations[0] * Vector3.up, 1f).magnitude;
					int num = 3;
					if (hasChest)
					{
						num++;
					...
					if (hasChest)
					{
						this.bones[this.chestIndex] = new IKSolverVR.VirtualBone(position, quaternion3);
					}
					if (hasNeck)
					{
						this.bones[this.neckIndex] = new IKSolverVR.VirtualBone(position2, rotation);
					}
					this.bones[this.headIndex] = new IKSolverVR.VirtualBone(vector3, quaternion4);
				...
				if (hasChest)
				{
					this.bones[this.chestIndex].Read(position, quaternion3);
				}
				if (hasNeck)
				{
					this.bones[this.neckIndex].Read(position2, rotation);
				}
				this.bones[this.headIndex].Read(vector3, quaternion4);
			...
			public void Solve(IKSolverVR.VirtualBone rootBone, IKSolverVR.Leg[] legs, IKSolverVR.Arm[] arms)
			{
				...
				this.Bend(this.bones, this.pelvisIndex, this.chestIndex, this.chestTargetRotation, this.chestClampWeight, false, this.chestRotationWeight);
